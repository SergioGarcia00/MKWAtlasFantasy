...
export async function POST(request: Request, { params }: { params: { playerId: string } }) {
    try {
        ...
        const player = ALL_PLAYERS.find(p => p.peak_mmr === parseInt(peakMmr));
        if (!player) {
            return NextResponse.json({ message: 'Player not found with that peak MMR' }, { status: 404 });
        }
